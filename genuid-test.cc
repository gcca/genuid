...
static constexpr std::size_t uids_size = 10000;

TEST(UID, MultiLockTier) {
  genuid::Init();
...

  {
    ...
    for (std::size_t i = 0; i < uids_size; i++) {
      gen_threads.emplace_back(
          [&uids](const std::size_t i) { uids[i] = genuid::LockTierUID(); }, i);
    }
  }
...
TEST(UID, MultiLockFree) {
...

  {
    ...
    for (std::size_t i = 0; i < uids_size; i++) {
      gen_threads.emplace_back(
          [&uids](const std::size_t i) { uids[i] = genuid::LockFreeUID(); }, i);
    }
  }
